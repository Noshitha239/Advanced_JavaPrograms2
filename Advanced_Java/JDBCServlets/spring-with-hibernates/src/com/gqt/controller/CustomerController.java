...
public class CustomerController {
	...
	public String updateCustomer(@RequestParam("name") String name) {//@RequestParam get the value in the name
		HibernateManager hm = new HibernateManager();
		int status=hm.updateName(name);
		if(status==1) {
	...
	public String updateCustomer1(@RequestParam("password") String password ) {//@RequestParam get the value in the name
		HibernateManager hm = new HibernateManager();
		int status=hm.updatePassword(password);
		if(status==1) {
